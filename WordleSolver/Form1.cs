...
{
    public partial class FormWordleSolver : Form
    {
        int FORM_HEIGHT = 150;
        int BUTTON_ORIGINAL_Y = 80;
        public FormWordleSolver()
        {
            InitializeComponent();
        }
        ...
        private string generatePotentialAnswers(string goodLetters, string badOrPotentialLetters, bool isBadLetters, string[] arrayOfPositions, bool isExpandedList = false)
        {
            ...
            if (!isBadLetters) //is potential letters array, convert to bad letters
            {
                newBadLetters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
                foreach (char c in badOrPotentialLetters.ToCharArray())
                {
                    newBadLetters.Replace(c.ToString(), string.Empty);
                }
            }

            //remove all the words that contain known bad characters
            foreach (string pAnswer in pAnswers)
            {
                bool remove = false;
                foreach(char badChar in badOrPotentialLetters.Trim().ToCharArray())
                {
                    if(pAnswer.ToUpper().Contains(badChar))
                    {
                        remove = true;
                        break;
                    }
                ...
                if(!remove)
                {
                    narrowedAnswersA.Add(pAnswer);
                }
            }

            //go through the list of potential solutions to check for the known good letters
            narrowedAnswersB.AddRange(narrowedAnswersA); //can't modify a list while enumerating it, so make a copy and modify copy
            foreach (char goodChar in goodLetters.ToCharArray())
            {
                foreach (string pAnswer in narrowedAnswersA)
                {
                    if(!pAnswer.ToUpper().Contains(goodChar))
                    {
                        narrowedAnswersB.Remove(pAnswer);
                    }
                    else
                    {
                        //check if the good character is in a known bad position
                    }
                }
            }
            narrowedAnswersA.Clear();
            narrowedAnswersA.AddRange(narrowedAnswersB);

            //remove all words that do not match known good positions
            for (int indexOutter = 0; indexOutter < 5 && indexOutter < goodLetters.Length; indexOutter++)//foreach(string positions in arrayOfPositions)
            {
                string positionString = arrayOfPositions[indexOutter];
                char goodLetter = goodLetters.ToCharArray()[indexOutter];
                for (int indexInner = 0; indexInner < 5; indexInner++) //for each position for the good letter...
                {
                    char position = positionString.ToCharArray()[indexInner];
                    if (position == 'Y') //if Y, then word is a potential match if the letter in that position matches goodLetter
                    {
                        foreach (string word in narrowedAnswersB)
                        {
                            if (word.ToUpper().ToCharArray()[indexInner] != goodLetter) //word is not a match, remove it from the list
                            {
                                narrowedAnswersA.Remove(word);
                            }
                        }
                    }
                    else if (position == 'X')//if X, then word is not a match if the letter in that position matches goodLetter
                    {
                        foreach (string word in narrowedAnswersB)
                        {
                            if (word.ToUpper().ToCharArray()[indexInner] == goodLetter) //word is not a match, remove it from the list
            ...
            //add the remaining potential answers to a string to return
            foreach (string pAnswer in narrowedAnswersA)
            {
                outString += pAnswer + Environment.NewLine;
            }
            return outString;
        }
        private void textBoxGoodLetters_TextChanged(object sender, EventArgs e)
        {
            TextBox[] textBoxes = { textBox1, textBox2, textBox3, textBox4, textBox5 };
            Label[] labels = { label1, label2, label3, label4, label5 };

            textBoxGoodLetters.Text = validateString(textBoxGoodLetters.Text);

            if (textBoxGoodLetters.Text.Length <= 5)
            {
                labelGoodInstructions.Visible = true;
                char[] knownGood = textBoxGoodLetters.Text.Trim().ToCharArray();
                for (int index = 0; index < knownGood.Length; index++)
                {
                    textBoxes[index].Visible = true;
                    labels[index].Visible = true;
                    labels[index].Text = knownGood[index] + ":";
                    textBoxes[index].Tag = knownGood[index];
                }
                for(int index = textBoxGoodLetters.Text.Trim().Length; index < 5; index++)
                {
                    textBoxes[index].Visible = false;
                    labels[index].Visible = false;
                    textBoxes[index].Tag = string.Empty;
                }

                labelGoodInstructions.Visible = textBoxGoodLetters.Text.Trim().Length != 0; //if length is 0, set to not visible
            }
            else
            {
                textBoxGoodLetters.Text = textBoxGoodLetters.Text.Substring(0, 5);
            }
            resizeFormForInput(textBoxGoodLetters.Text.Length);
        }

        private void radioButtonGoodLetters_CheckedChanged(object sender, EventArgs e)
        {
            radioButtonBadLetters.Checked = !radioButtonGoodLetters.Checked;
            radioButtonsHighlight();
        }

        private void radioButtonBadLetters_CheckedChanged(object sender, EventArgs e)
        {
            radioButtonGoodLetters.Checked = !radioButtonBadLetters.Checked;
            radioButtonsHighlight();
        }

        private void radioButtonsHighlight()
        {
            if(radioButtonBadLetters.Checked)
            {
                radioButtonBadLetters.Font = new Font(radioButtonBadLetters.Font, FontStyle.Bold);
                radioButtonGoodLetters.Font = new Font(radioButtonGoodLetters.Font, FontStyle.Regular);
            }
            else
            {
                radioButtonGoodLetters.Font = new Font(radioButtonGoodLetters.Font, FontStyle.Bold);
                radioButtonBadLetters.Font = new Font(radioButtonBadLetters.Font, FontStyle.Regular);
            }
        ...
        private string validateString(string instring)
        {
            string outString = string.Empty;
            char[] charString = instring.ToCharArray();
            foreach(char c in charString)
            {
                if(isLetter(c) && !outString.Contains(c)) //make sure of no duplicates and only letters
                {
                    outString += c;
                }
            }
        ...
        {
            textBoxLetters.Text = validateString(textBoxLetters.Text);
        }

        private void resizeFormForInput(int inputsCount)
        {
            FormWordleSolver.ActiveForm.Height = FORM_HEIGHT + (inputsCount * 22);
            buttonSolve.Location = new Point(buttonSolve.Location.X, BUTTON_ORIGINAL_Y + (inputsCount * 22));
        }

        private void FormWordleSolver_Shown(object sender, EventArgs e)
        {
            FormWordleSolver.ActiveForm.Height = FORM_HEIGHT;
        }

        private string validateGoodLetterPositionString(string positions)
        {
            string outString = string.Empty;
            foreach(char c in positions.Trim().ToCharArray())
            {
                if ("XY?".Contains(c))
                {
                    outString += c;
        ...
        private void textBoxGoodPositions_TextChanged(object sender, EventArgs e)
        {
            ((TextBox)sender).Text = validateGoodLetterPositionString(((TextBox)sender).Text);
        }
    }
}
